...
$(function () {
    ...
    const get_next_fn = function () {
        ...
        return new Promise((resolve) => {
            $.ajax({
                url: '/assess/aesth/get?' + $.param({token: token}),
                type: 'get',
                cache: false,
                timeout: 60000,
                success: function (data) {
                    ...
                    if (data['fid'] !== 'null') {
                        ...
                        img_url = '/assess/tech/file?s=' + data['file']['storage'] + '&p=' + data['file']['path'] + '&f=' + data['file']['name'];
                    }
    ...
    const mark_fn = async function (type, fid) {
        ...
        $.ajax({
            url: '/assess/aesth/mark?'  + $.param({fid: fid, type: type, token: token}) ,
            type: 'post',
    ...
    function loadStatistics() {
        $.ajax({
            url: '/assess/tech/stats?' + $.param({token: token}),
            type: 'get',
            success: function(data) {
    ...
    // Button 1
    $('#btn_1').click(function(evt) {
        mark_fn('1', current_fid);
        if (evt) evt.preventDefault();
        return false;
    });

    // Button 2
    $('#btn_2').click(function(evt) {
        mark_fn('2', current_fid);
        if (evt) evt.preventDefault();
        return false;
    });

    // Button 3
    $('#btn_3').click(function(evt) {
        mark_fn('3', current_fid);
        if (evt) evt.preventDefault();
        return false;
    });

    // Button 4
    $('#btn_4').click(function(evt) {
        mark_fn('4', current_fid);
        if (evt) evt.preventDefault();
        return false;
    });
    ...
    $('#btn_0').click(function(evt) {
        mark_fn('inappropriate', current_fid);
        if (evt) evt.preventDefault();
        return false;
    });
