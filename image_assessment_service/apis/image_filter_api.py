...
image_filter_api = APIRouter()

@image_filter_api.on_event("startup")
async def startup():
    ...
    _startup_ran = True
    db = SessionLocal()
    service = SamplesService(code="filter", db=db)

@image_filter_api.get('/get')
async def get(token: str, db: Session = Depends(get_db), code: str = "filter"):
...
@image_filter_api.get('/file')
def get_file(s: str, p: str, f: str):
    full_path = Storages().get(code=s).to_path(p, f)
    return FileResponse(full_path)

@image_filter_api.post('/mark')
def mark_yes(type: str, fid: str, token: str, db: Session = Depends(get_db), code: str = "filter"):
    if fid is None or len(fid) == 0:
        return
    service.mark(fid, type, code=code, db=db, token=token)

@image_filter_api.get('/stats')
